...
from fastai.vision import cnn_learner, ImageDataBunch
from fastai import *
...
class EmotionDetectionUsingSpectrogram(nn.Module):
    def __init__(self, fastai_model_path):
        ...
        # WE NEED TO TRAIN THIS MODEL FIRST WITH ONLY 6 EMOTIONS AND THEN USE IT HERE.
        # labels_literal = 
        # data = ImageDataBunch.from_lists(".", image_path_list, 
        # pretrained_model = cnn_learner(
        base_model = model.resnet34()
        base_model.load_state_dict(torch.load(fastai_model_path))

        # Freeze all layers
        for param in base_model.parameters():
            param.requires_grad = False

        # Get the number of features in the last layer of base model
        num_features = base_model.fc.in_features
        # Define a new linear layer with the same number of input features
        # and the desired number of output classes (e.g., 6 for 6 classes)
        self.new_layer = nn.Linear(num_features, 6)  # Assuming 6 classes for classification
        # Concatenate the base model with the new layer
        self.model = nn.Sequential(base_model, self.new_layer)

        def forward(self, x):
            # Pass the input through the base model and the new layer
            # Add the output of the new layer to the existing logits
            return self.model(x) + x  # Assuming x contains the logits
...
class MentalModel(nn.Module):
    ...
    def __init__(self, encoder_type='bert-base-uncased', encoder_mode='maxpooling', sent_dim=300, tr_nhead=6, tr_ff_dim=300, tr_dropout=0.1, attn_mask=True, tr_num_layer=6, max_len=120, num_class=6, bidirectional=True, num_block=3, cn_nhead=6, cn_ff_dim=300, cn_dropout=0.1, edge_dim=300, bias=False, cn_num_layer=3, edge_mapping=True, beta=True, root_weight=True, choice='both'):
        ...
        TRANSFER_LEARNING_MODELPATH = "/content/gdrive/MyDrive/emotion_detection_using_sound/saved_models/emotion_detection_using_only_sound/emotion_detection_model_using_spectrogram_4th_iter"
        self.choice = choice
        ...
        if choice == 'both':
            ...
            self.spectrogram_classifier = EmotionDetectionUsingSpectrogram(TRANSFER_LEARNING_MODELPATH)
    ...
    def forward(self, conversation, mask, utt_mask=None, spk_mask=None,
                window=None, mode=None, edge_index=None, edge_attr=None, image_path_list=None, residual=False):
